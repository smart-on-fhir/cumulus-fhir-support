...
class BadAuthArguments(AuthError):
    """Raised when there are conflicting auth parameters"""


class AuthFailed(AuthError):
    """Raised when authentication did not succeed"""
...
def urljoin(base: str, path: str) -> str:
    ...
    if not base:
        raise BadAuthArguments("You must provide a FHIR server URL.")
...
class Auth:
    ...
    async def authorize(self, session: httpx.AsyncClient, reauthorize=False) -> None:
        ...
        if reauthorize:
            ...
            raise BadAuthArguments("Could not connect to server without authentication arguments")
...
class JwtAuth(Auth):
    ...
    async def authorize(self, session: httpx.AsyncClient, reauthorize=False) -> None:
        ...
        # Use v1 scopes unless the server says it only uses v2
    ...
    async def _ensure_config(self, session: httpx.AsyncClient) -> None:
        ...
        if self._token_url:
            self._config["token_endpoint"] = self._token_url
        ...
        if not self._config.get("token_endpoint"):
            raise AuthFailed(f"Server {self._server_root} does not expose an OAuth token endpoint")
    ...
    def _make_pem_jwk(self) -> tuple[str, jwk.JWK]:
        ...
        raise BadAuthArguments("No supported private key found in the provided PEM file.")
    ...
    def _make_jwks_jwk(self) -> tuple[str, jwk.JWK]:
        ...
        raise BadAuthArguments("No valid private key found in the provided JWKS file.")
...
class BasicAuth(Auth):
    ...
    def __init__(self, user: str, password: str):
        super().__init__()
        # Assume utf8 is acceptable -- we should in theory also run these through Unicode
        # normalization, in case they have interesting Unicode characters. But we can always add
        # that in the future.
...
def create_auth(
    ...
    if total_auth_types > 1:
        raise BadAuthArguments(
            "Multiple authentication methods have been specified. Double check your arguments."
        )
    ...
    if basic_user and basic_password:
        ...
        raise BadAuthArguments(
            "You must provide both a Basic user and a Basic password "
    ...
    if smart_client_id and has_smart_args:
        return JwtAuth(server_root, token_url, smart_client_id, smart_jwks, smart_pem, resources)
    elif smart_client_id or has_smart_args:
        raise BadAuthArguments(
            "You must provide both a SMART client ID and a SMART key "
            "to connect to a SMART FHIR server."
        )

    # Fall back to the unauthenticated base class
    return Auth()
